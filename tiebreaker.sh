...
sed -e "s/host1-need-config/$DRBD_HOST1_NAME/g" -e "s/host2-need-config/$DRBD_HOST2_NAME/g" \
  ...
  -e "s%disk1-need-config%$DRBD_DISK1%g" -e "s%disk2-need-config%$DRBD_DISK2%g" /etc/keepalived/etcd-drbd-template.res >/etc/drbd.d/etcd.res
...
sed -e "s%ip1-need-config%$DRBD_IP1%g" -e "s%ip2-need-config%$DRBD_IP2%g" \
  -e "s%vip-need-config%$VIP%g" /etc/keepalived/start-etcd-template.sh >/etc/keepalived/start-etcd.sh && chmod +x /etc/keepalived/start-etcd.sh

echo "config keepalived"
sed -e "s/vip_need_config/$VIP/g" -e "s/dev_need_config/$NET/g" /etc/keepalived/keepalived-template.conf >/etc/keepalived/keepalived.conf
